...
/*classk pa gpio control begin*/
unsigned int CLASSK_PIN_ENABLE;
static const struct  of_device_id classk_of_match[] = {
	{
	.compatible = "classk,classk_aw8736",
	.data = 0,
    },
	{},
};

static int classk_probe(struct platform_device *pdev)
{
	const struct of_device_id *id;
	id = of_match_node(classk_of_match,pdev->dev.of_node);
	if(!id)
		 return -1;
	CLASSK_PIN_ENABLE = of_get_named_gpio(pdev->dev.of_node,"classk_en_pin",0);
	gpio_request(CLASSK_PIN_ENABLE,"CLASSK_EN_PIN");
	return 0;
	
}

static struct platform_driver classk_driver = {
	.probe = classk_probe,
	.driver = {
			.name="classk_aw8736",
			.owner = THIS_MODULE,
			.of_match_table = classk_of_match,
	},
};
static int __init classk_init(void)
{
 if(platform_driver_register(&classk_driver)){
 	pr_debug("classk fail to register this driver\n");
	return -1;

 }
 return 0;
}
static void __exit classk_exit(void)
{

}
late_initcall(classk_init);
module_exit(classk_exit);
MODULE_AUTHOR("mediatek");
MODULE_DESCRIPTION("classk subsystem driver");
MODULE_LICENSE();

/*classk pa gpio control end*/
 /*classk switch*/
/*0.75us ~ 10us*/
static void class_k_switch(unsigned GPIO,unsigned int enable)
{
	if(enable){
...
static void Speaker_Amp_Change(bool enable)
{
	if (enable) {
		...
		/*classk enable*/
		class_k_switch(CLASSK_PIN_ENABLE,1);
	} else {
		pr_debug("%s(), enable %d\n", __func__, enable);
		/* LOL mux to open */
		...
		if (GetDLStatus() == false) {
			...
			TurnOffDacPower();
			/*classk disable*/
			class_k_switch(CLASSK_PIN_ENABLE,0);
		}
	}
}
...
static void Headset_Speaker_Amp_Change(bool enable)
{
	if (enable) {
        ...
        //Modified by wanying.chen fix PR6500547 on 2018/06/29 begin
        class_k_switch(CLASSK_PIN_ENABLE,1);
        //Modified by wanying.chen fix PR6500547 on 2018/06/29 end
#ifndef ANALOG_HPTRIM
        ...
        //Modified by wanying.chen fix PR6500547 on 2018/06/29 begin
        class_k_switch(CLASSK_PIN_ENABLE,0);
        //Modified wanying.chen fix PR6500547 on 2018/06/29 end
