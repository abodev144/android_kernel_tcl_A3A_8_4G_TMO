...
#include "fsunhi556mipiraw_Sensor.h"
#include "../imgsensor_proc.h"

/*===FEATURE SWITH===*/
 // #define FPTPDAFSUPPORT   //for pdaf switch
#define PFX "front_sunrise_hi556_camera_sensor"
...
static void preview_setting(kal_uint16 currefps)
{
	...
	if( currefps == 300 )
	{
		pr_err("hi556 preview_setting fps = 300\n");
		write_cmos_sensor(0x0b0a, 0x8252);
		write_cmos_sensor(0x0f30, 0x6e25);
		write_cmos_sensor(0x0f32, 0x7067);
		write_cmos_sensor(0x004a, 0x0100);
	...
	{
		pr_err("hi556 preview_setting fps not 300\n");
		//Sensor Information////////////////////////////
		//Sensor	  : Hi-556
		//Date		  : 2016-10-19
...
static void slim_video_setting(void)
{
	...
	//BLC offset	  : 64code
	////////////////////////////////////////////////
  pr_err("hi556 slim_video_setting");
	write_cmos_sensor(0x0a00, 0x0000);
	write_cmos_sensor(0x0b0a, 0x8252);
	write_cmos_sensor(0x0f30, 0x6e25);
...
int sensor_id_otp=0,lens_id=0,vcm_id=0,vcm_ic_id=0,f_number=0;
int check_sum=0,check_sum_cal=0;
int data[17]={0};
int data_awb[30]={0};
int i=0;
int awb_flag=0,awb_start_addr=0;
//int RGr=0,BGb=0,GbGr=0;
//int gRGr=0,gBGb=0,gGbGr=0;
...
otp_flag=read_cmos_sensor_byte(0x0108);
...
LOG_INF("sensor_id_otp=0x%x,lens_id=0x%x,vcm_id=0x%x,vcm_ic_id=0x%x\n",sensor_id_otp,lens_id,vcm_id,vcm_ic_id);
LOG_INF("f_number=0x%x,check_sum=0x%x,check_sum_cal=0x%x\n",f_number,check_sum,check_sum_cal);

write_cmos_sensor_8(0x010a,((0x0435)>>8)&0xff);
write_cmos_sensor_8(0x010b,(0x0435)&0xff);
write_cmos_sensor_8(0x0102,0x01);
awb_flag=read_cmos_sensor_byte(0x0108);
if(awb_flag==0x01)
...
write_cmos_sensor_8(0x0102,0x01);
	for(i=0;i<=29;i++)
	{
	data_awb[i]=read_cmos_sensor_byte(0x0108);
...
RGr=((data_awb[0]<<8)&0xff00)|(data_awb[1]&0xff);
BGb=((data_awb[2]<<8)&0xff00)|(data_awb[3]&0xff);
GbGr=((data_awb[4]<<8)&0xff00)|(data_awb[5]&0xff);
gRGr=((data_awb[6]<<8)&0xff00)|(data_awb[7]&0xff);
...
LOG_ERR("gRGr=0x%x,gBGb=0x%x,gGrGb=0x%x\n",gRGr,gBGb,gGbGr);
LOG_INF("check_sum_awb=0x%x,check_sum_awb_cal=0x%x\n",check_sum_awb,check_sum_awb_cal);
if((otp_flag!=0)&&(awb_flag!=0)&&(module_id==0x44)&&(lens_id==0xb4)&&(check_sum==check_sum_cal)&&(check_sum_awb==check_sum_awb_cal))
{
	hi556_otp_flag=1;
}
else
...
{    
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_ERR("front sunrise hi556 [get_imgsensor_id] ");
...
UINT32 FRONT_SUNRISE_HI556_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
