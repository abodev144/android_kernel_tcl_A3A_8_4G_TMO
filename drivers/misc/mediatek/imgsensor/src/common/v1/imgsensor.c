...
/*Add-Begin-By falin.luo for camera robust test */
#ifdef CONFIG_TCT_ROBUST
extern void register_camera_robust_dev(unsigned int sensor_dev);
#endif
/*Add-End-By falin.luo for camera robust test */
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {	  /* not implement this feature ID */
        ...
        /*Add-Begin-By falin.luo for camera robust test */
        #ifdef CONFIG_TCT_ROBUST
        register_camera_robust_dev((unsigned int)psensor->inst.sensor_idx + 1);
        #endif
        /*Add-End-By falin.luo for camera robust test */
		err = ERROR_NONE;
	}
 ...
 * imgsensor_init()
 */
static unsigned int CAMID0;
static unsigned int CAMID1;

unsigned int get_camera_id0(void)
{
	return gpio_get_value(CAMID0);
}
EXPORT_SYMBOL(get_camera_id0);

unsigned int get_camera_id1(void)
{
	return gpio_get_value(CAMID1);
}
EXPORT_SYMBOL(get_camera_id1);

static int cam_id_driver_probe(struct device *dev, void const *data)
{
	CAMID0 = of_get_named_gpio(dev->of_node, "camid0", 0);
	gpio_request(CAMID0, "camid0");
	gpio_direction_input(CAMID0);

	CAMID1 = of_get_named_gpio(dev->of_node, "camid1", 0);
	gpio_request(CAMID1, "camid1");
	gpio_direction_input(CAMID1);

	return 0;
}

static const struct of_device_id cam_id_platform_of_match[] = {
	{
	 .compatible = "camera,cameraid_detect",
	}, 
	{}
};
...
static int cam_id_platform_probe(struct platform_device *pdev)
{
	const struct of_device_id *id;
	id = of_match_node(cam_id_platform_of_match, pdev->dev.of_node);
	if (!id)
		return -ENODEV;
	return cam_id_driver_probe(&pdev->dev, id->data);
}

static struct platform_driver cam_id_driver = {
	.probe = cam_id_platform_probe,
	.driver = {
		   .name = "cameraid_detect",
		   .owner = THIS_MODULE,
		   .of_match_table = cam_id_platform_of_match,
		   },
};
...
static int __init imgsensor_init(void)
{
	...
	if(platform_driver_register(&cam_id_driver))
		PK_PR_ERR("failed to register cameraid detect\n");
...
static void __exit imgsensor_exit(void)
{
	...
	platform_driver_unregister(&cam_id_driver);
}
