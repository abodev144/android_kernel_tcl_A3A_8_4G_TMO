 ...
 *historyo1  Modified by lanying.he for XR5592217 on 2017/12/24 for significant 
 *
 *****************************************************************************/
...
static int	MC3XXX_ReadData(struct i2c_client *pt_i2c_client, s16 waData[MC3XXX_AXES_NUM])
{
	...
	if (s_bPCODE == MC3XXX_PCODE_3250) {
		...
		if (s_bMPOL & 0x01)
			waData[MC3XXX_AXIS_X] = waData[MC3XXX_AXIS_X]; //- lzk
		if (s_bMPOL & 0x02)
...
int mc3413_set_acc_interrupter(bool flag)
{
	...
	if(MC3XXX_i2c_read_block(obj->client, MC3XXX_REG_TILT_STATUS, databuf, 0x01)<0) 
	{ 
		ACC_PR_ERR("%s read MC3XXX_REG_TILT_STATUS register err!\n", __func__); 

	} 
	enable_irq(mc3xxx_obj_i2c_data->irq);  //liupeng
	ACC_PR_ERR("%s read MC3XXX_REG_TILT_STATUS register =%x!\n", __func__,databuf[0]); 
	MC3XXX_i2c_read_block(obj->client, 0x06, databuf, 1); 
	ACC_PR_ERR("%s read 0x06 register =%x!\n", __func__,databuf[0]); 
	MC3XXX_i2c_read_block(obj->client, 0x09, databuf, 1); 
	ACC_PR_ERR("%s read 0x09 register =%x!\n", __func__,databuf[0]); 
    }
    else
    {
