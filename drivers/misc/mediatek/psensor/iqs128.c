...
int iqs128_irq;
static unsigned int iqs128_eint_gpio;
static unsigned int eint_debounce;
static int iqs128_int_value = 0;
static struct workqueue_struct *iqs128_wq;
static struct work_struct *iqs128_work;

static struct pinctrl *iqs128_eint_pinctrl;
static struct pinctrl_state *iqs128_eint_default;
static struct pinctrl_state *iqs128_eint_active;
static struct pinctrl_state *iqs128_power_en_enable;
static struct pinctrl_state *iqs128_power_en_disable;

static struct of_device_id iqs128_of_match[] = {
    {.compatible = "mediatek,psensor"},
    {},
};
...
ssize_t psensor_power_en_store(struct file *file, const char __user *buf,
                               size_t size, loff_t *ppos)
{
    char value[16] = {0};

    if (copy_from_user(&value, buf, size)) {
        printk(KERN_CRIT"%s:Failed to copy args from user sapce!\n", __func__);
        return -EFAULT;
    }

    if (!strncmp(value, "on", 2) && psensor_power_on == 0) {
    ...
    } else if (!strncmp(value, "off", 3) && psensor_power_on == 1) {
...
/*modify(add) by zhikui.li,Change the sar sensor release function begin */
    .release = single_release,//seq_release,
/*modify(add) by zhikui.li,Change the sar sensor release function  end */
...
int iqs128_pinctrl_init(struct device *dev)
{
    ...
    iqs128_power_en_enable = pinctrl_lookup_state(iqs128_eint_pinctrl,
                             "power_en_enable");
    ...
    iqs128_power_en_disable = pinctrl_lookup_state(iqs128_eint_pinctrl,
                              "power_en_disable");
...
static irqreturn_t iqs128_int_handler(void)
{
    ...
    iqs128_int_value = __gpio_get_value(iqs128_eint_gpio);
    printk(KERN_CRIT"%s: iqs128_int_value = %d\n", __func__, iqs128_int_value);
    //irq_set_irq_type(iqs128_irq, iqs128_int_value ? IRQ_TYPE_EDGE_FALLING : IRQ_TYPE_EDGE_RISING);
...
static int iqs128_probe(struct platform_device *dev)
{
    ...
    proc_name = proc_create("power_en", 0666, proc_psensor_dir,
                            &psensor_power_en_fops);
    if (proc_name == NULL) {
    ...
    if (node) {
        of_property_read_u32(node, "debounce", &eint_debounce);
        iqs128_eint_gpio = of_get_named_gpio(node, "interrupt-gpios", 0);
        printk(KERN_CRIT"enint_gpio = %d, debounce time = %d\n",
               iqs128_eint_gpio, eint_debounce);
        gpio_direction_input(iqs128_eint_gpio);
        gpio_set_debounce(iqs128_eint_gpio, eint_debounce);
        iqs128_irq = irq_of_parse_and_map(node, 0);
        //ret = request_irq(iqs128_irq, (irq_handler_t)iqs128_int_handler,
        //                  gpio_get_value(iqs128_eint_gpio) ? IRQ_TYPE_EDGE_FALLING :
        //                  IRQ_TYPE_EDGE_RISING, "iqs128", NULL);
        ret = request_irq(iqs128_irq, (irq_handler_t)iqs128_int_handler,
                          IRQ_TYPE_EDGE_BOTH, "iqs128", NULL);
        if (ret != 0) {
