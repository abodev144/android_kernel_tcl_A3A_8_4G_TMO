...
static struct firmware gt9xx_firmware = {
    .data = &gt9xx_fw[0],
    .size = sizeof(gt9xx_fw),
};
u16 show_len;
u16 total_len;
u8 searching_file = 0;
                                ...
                                struct st_fw_head *fw_head, u8 *path)
{
    ...
    update_msg.fw_total_len = update_msg.file->f_op->llseek(update_msg.file, 0,
                              SEEK_END);
                        ...
                        u16 total_length)
{
    ...
    while (burn_length < total_length) {
        ...
        frame_length =
            ((total_length - burn_length) > PACK_SIZE) ? PACK_SIZE :
                           ...
                           u16 start_rd_addr, u16 chk_length)
{
    ...
    while (recall_length < chk_length) {
        frame_length =
            ((chk_length - recall_length) > PACK_SIZE) ? PACK_SIZE :
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
    ...
    ret = gup_load_section_file(fw_dsp_isp, FW_DSP_ISP_LENGTH, FW_DSP_ISP_LENGTH,
                                SEEK_END);
    if (FAIL == ret) {
...
static u8 gup_burn_fw_link(struct i2c_client *client)
{
    ...
    ret = gup_load_section_file(fw_link, offset, FW_LINK_LENGTH - FW_SECTION_LENGTH,
                                SEEK_SET);
    if (FAIL == ret) {
...
static u8 gup_burn_fw_app_code(struct i2c_client *client)
{
    ...
    ret = gup_burn_fw_app_section(client, fw_app_code, 0x9000, FW_SECTION_LENGTH,
                                  0x3A);
    if (FAIL == ret) {
    ...
    ret = gup_load_section_file(fw_app_code, start_index + 2 * FW_SECTION_LENGTH,
                                FW_SECTION_LENGTH, SEEK_SET);
    if (FAIL == ret) {
...
s32 gup_reload_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
...
#else
    update_msg.fw = &gt9xx_firmware;
#endif

    GTP_DEBUG("Begin reload dsp fw for gesture wakeup mode!");

    /* step1:enter update mode */
    GTP_DEBUG("step1:enter update mode");
    ret = gup_enter_update_mode_fl(client);
    if (ret == FAIL) {
    ...
    /* step3:write the second bank */
    GTP_DEBUG("step3:write the second bank");
    ret = i2c_write_bytes(client,
                          0xC000,
                          (u8 *)&update_msg.fw->data[FW_HEAD_LENGTH + 2 * FW_DOWNLOAD_LENGTH],
                          FW_DSP_LENGTH);
    ...
    ret = i2c_write_bytes(client, 0x801F, &value, 1);
    if (ret == -1) {
        ret = FAIL;
        GTP_ERROR("failed to reload fw dsp!");
    } else {
        ret = SUCCESS;
        GTP_INFO("success to reload fw dsp!");
    }

exit_download_fw_dsp:
#ifdef CONFIG_GTP_REQUEST_FW_UPDATE
    if (update_msg.fw)
        firmware_release(update_msg.fw);
#endif
    update_msg.fw = NULL;
    return ret;
}
