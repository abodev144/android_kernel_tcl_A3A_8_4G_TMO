...
s32 gesture_event_handler(struct input_dev *dev)
{
    ...
    if (DOZE_ENABLED == gesture_data.doze_status) {
        ...
        if (ret == 0 && doze_buf[0] != 0) {
            ...
            if (len > 0) {
                ret = ges_i2c_read_bytes(GTP_REG_WAKEUP_GESTURE_DETAIL, &gesture_data.data[4],
                                         len * 4);
                if (ret < 0) {
            ...
            if (extra_len > 0) {
                ret = ges_i2c_read_bytes(GTP_REG_WAKEUP_GESTURE + 4,
            ...
            gesture_data.doze_status = DOZE_WAKEUP;
            mutex_unlock(&gesture_data_mutex);
