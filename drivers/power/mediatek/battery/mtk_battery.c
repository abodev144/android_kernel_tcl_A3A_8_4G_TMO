...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20180314,task6052905
//add battery type attribute in battery power supply
	POWER_SUPPLY_PROP_BATTERY_TYPE,
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20180314,task6052905
};
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20180327,task6052524
//modify get BatteryAverageCurrent interface for user space
	//bat_data->BAT_BatteryAverageCurrent = battery_get_ibus();
	bat_data->BAT_BatteryAverageCurrent = pmic_get_charging_current();
	
	if((mt_get_charger_type() == STANDARD_HOST) && (bat_data->BAT_BatteryAverageCurrent > 500))
		bat_data->BAT_BatteryAverageCurrent = 500;
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20180327,task6052524	
	bat_data->BAT_ISenseVoltage = battery_meter_get_VSense();
	bat_data->BAT_ChargerVoltage = battery_get_vbus();

	// add start by TCT-cuiping.shi for batt status error!!
	if( (bat_data->BAT_STATUS==POWER_SUPPLY_STATUS_CHARGING) &&
		(bat_data->BAT_ChargerVoltage<4000))
		bat_data->BAT_STATUS=POWER_SUPPLY_STATUS_DISCHARGING;
	// add stop by TCT-cuiping.shi for batt status error!!
}
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	// add start by TCT-cuiping.shi for update battery status
	if(POWER_SUPPLY_PROP_PRESENT==psp)
		battery_update_psd(data);
	// add stop by TCT-cuiping.shi for update battery status
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		#if 0 // change start by TCT-cuiping.shi for current LOG
		if (b_ischarging == false)
		#else
		if (b_ischarging)
		#endif// change stop by TCT-cuiping.shi for current LOG
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20180514,task6313649		
//update BatteryAverageCurrent when read this interface 	
		if(data->BAT_BatteryAverageCurrent < 300){
			data->BAT_BatteryAverageCurrent = pmic_get_charging_current();
			if((mt_get_charger_type() == STANDARD_HOST) && (data->BAT_BatteryAverageCurrent > 500))
				data->BAT_BatteryAverageCurrent = 500;			
		}
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20180514,task6313649			
...
//add battery type attribute in battery power supply
	case POWER_SUPPLY_PROP_BATTERY_TYPE:
		data->BAT_Battery_Type = g_fg_battery_id;
		val->intval = data->BAT_Battery_Type;
		break;
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20180314,task6052905
		/* Dual battery */
...
*/
	if (id_volt < g_battery_id_voltage[0]) {
			g_fg_battery_id = 1;//BYD
	} else if (id_volt < g_battery_id_voltage[1]) {
			g_fg_battery_id = 2;//VK
	}else{
			g_fg_battery_id = 0;//JN
	}
...
int BattThermistorConverTemp(int Res)
{
	...
	bm_err("[BattThermistorConverTemp] %d %d %d %d %d %d\n", RES1, RES2, Res, TMP1, TMP2, TBatt_Value);
...
int force_get_tbat(bool update)
{
	...
	bm_debug("[force_get_tbat] MMI SW TBAT=25 t\n");
...
void fg_drv_update_hw_status(void)
{
	...
	if((tmp <= 10) && (mt_get_charger_type() == CHARGER_UNKNOWN)){
		fg_cust_data.car_tune_value = LOW_TEMP_DIS_CAR_TUNE_VALUE * UNIT_TRANS_10;
	}else if((tmp <= 10) && (mt_get_charger_type() != CHARGER_UNKNOWN)){		
		fg_cust_data.car_tune_value = LOW_TEMP_CHR_CAR_TUNE_VALUE * UNIT_TRANS_10;
	}else{
		fg_cust_data.car_tune_value = CAR_TUNE_VALUE * UNIT_TRANS_10;
	}
		
	bm_err("AvgVbat=%d,VChr=%d,T=%d,Soc=%d,UI_SOC:%d,CT:%d,car_tune_value:%d\n",
			bat_vol, chr_vol, tmp, battery_get_bat_soc(), battery_get_bat_uisoc(), mt_get_charger_type(), fg_cust_data.car_tune_value);
...
//modify NTC ,always use 10s update 
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20180319,task6113677
//add otg and charger_type interface for user space
static ssize_t show_is_otg_mode(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int is_otg_mode;

	if (mt_usb_is_device()) {
		is_otg_mode = 0;
		bm_err( "[otg mode detect]USB is not in otg mode.");
	} else {
		is_otg_mode = 1;
		bm_err("[otg mode detect]USB is in otg mod.");
	}

	bm_err("is_otg_mode = %d\n", is_otg_mode);
	return sprintf(buf, "%d\n", is_otg_mode);
}

static ssize_t store_is_otg_mode(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	bm_err("[otg mode detect] Not Support Write Function\n");
	return size;
}

static DEVICE_ATTR(is_otg_mode, 0664, show_is_otg_mode, store_is_otg_mode);
...
static ssize_t show_Charger_Type(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	chr_ype = mt_get_charger_type();
	bm_err("CHARGER_TYPE = %d\n", chr_ype);
...
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20180319,task6113677
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20180319,task6113677
//add otg and charger_type interface for user space	
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_is_otg_mode);
