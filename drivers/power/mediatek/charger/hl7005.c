...
static int hl7005_get_cv_voltage(struct charger_device *chg_dev, u32 *cv)
{
	unsigned char reg6 = 0;	
    int status = 0;
	
	hl7005_read_byte(0x06,&reg6, 1);
	switch(reg6 & 0x0f){
	case 0: 
	    	*cv = 4200; 
		break;
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20180321,task6138420
//add max charging voltage node for user space 
	.get_constant_voltage = hl7005_get_cv_voltage,
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20180321,task6138420
...
static int hl7005_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hl7005_reg_config_interface(0x04, 0x1B);	/* 200mA */
