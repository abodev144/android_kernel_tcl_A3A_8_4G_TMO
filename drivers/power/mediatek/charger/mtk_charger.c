...
//modify NTC ,for distinguish 1A or 2A charger current
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20180416,task6052908
//modify NTC ,for distinguish 1A or 2A charger current	
	gpio_current_select = of_get_named_gpio(np, "gpio_charging_current_select", 0);
	
	if (gpio_current_select < 0){
		chr_err("get gpio_charging_current_select failed \n");
	}
	else{
		gpio_direction_input(gpio_current_select);
		msleep(20);
		val = gpio_get_value(gpio_current_select);
		chr_err("get gpio_charging_current_select %d:%d \n", gpio_current_select, val);	
		
		if(val)
			info->data.ac_charger_current = 1050000;
		else
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20180321,task6138420
//add max charging voltage node for user space 
static ssize_t show_max_charging_voltage(struct device *dev, struct device_attribute *attr, char *buf)
{
	int cv;
	struct charger_manager *pinfo = dev->driver_data;
	
	charger_dev_get_constant_voltage(pinfo->chg1_dev, &cv);
	
	return sprintf(buf, "%d\n", cv);
}

static DEVICE_ATTR(max_charging_voltage, 0664, show_max_charging_voltage, NULL);
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20180321,task6138420
/* procfs */
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20180312,task6052539
//modify the function that disable charging and control the charging current
		g_current_cmd_set = cmd_current_unlimited;
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20180312,task6052539
...
/* Add-Begin falin.luo add node to enable/disable usb charger */
static ssize_t show_chg_enable(struct device *dev, struct device_attribute *attr,
        char *buf)
{
    struct charger_manager *info = dev->driver_data;
    chr_err("show_chg_enable %d\n",!info->cmd_discharging);
    return sprintf(buf, "%u\n", !info->cmd_discharging);
}
        ...
        const char *buf, size_t size)
{
    struct charger_manager *info = dev->driver_data;
    int enable = 0;
    chr_err("[EM] store_chg_enable\n");
    if (kstrtouint(buf, 10, &enable) == 0) {
        chr_err("chg_enable = %d\n", enable);
        info->usb_unlimited = false;
        if (enable == 0) {
            charger_dev_enable(info->chg1_dev, false);
...
/* Add-End falin.luo add node to enable/disable usb charger */
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
    ...
    /* Add-Begin falin.luo add node to enable/disable usb charger */
    ret = device_create_file(&(pdev->dev), &dev_attr_chg_enable);
    if (ret)
        goto _out;
    /* Add-End falin.luo add node to enable/disable usb charger */
...
//[BUGFIX]-Mod-BEGIN by SCDTABLET.zhengding.chen@tcl.com,20180321,task6138420
//add max charging voltage node for user space 
	ret = device_create_file(&(pdev->dev), &dev_attr_max_charging_voltage);
	if (ret)
		goto _out;
//[BUGFIX]-Mod-END by SCDTABLET.zhengding.chen@tcl.com,20180321,task6138420	
